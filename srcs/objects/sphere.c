...
double	sphere_intersection(t_minirt *minirt, t_ray ray, t_object sphere)
{
	...
	double b = 2.0f * dot_vec3(offset_ray_origin, ray.direction); // ou off set ray origin / ray.origin ?
	double c = dot_vec3(offset_ray_origin, offset_ray_origin) - sphere.spec.sphere.radius * sphere.spec.sphere.radius;
	double discriminant = b * b - 4.0f * a * c;
	if (discriminant < 0.f) 
		return (-1);
	// colorisation de la scene
	double t0 = (-b - sqrt(discriminant)) / (2.0f * a); // calcul de la distance entre le point d'origine et le point d'intersection
